...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await prisma.accountingConnection.upsert({
      ...
      update: { accessToken: apiKey, refreshToken: '', enabled: true, expiresAt: new Date(Date.now() + 365 * 24 * 3600 * 1000) },
      create: { orgId: u.orgId, provider, accessToken: apiKey, refreshToken: '', enabled: true, expiresAt: new Date(Date.now() + 365 * 24 * 3600 * 1000) },
    });

    return res.status(200).json({ ok: true });
  } catch (e: any) {
    console.error('admin/integrations/accounting key error', e?.message || e);
